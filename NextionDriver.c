/*
 *   Copyright (C) 2017...2019 by Lieven De Samblanx ON7LDS &  Mariano Sånchez EA5KL
 *
...
//                } else
                ...
                if (RXbuffer[1]==0xA0){
		    sprintf(TXbuffer, "page=\"LastHeard\"", buf);
		    sendCommand(TXBuffer);
		    sprintf(TXbuffer, "LHt1.txt=\"Botón pulsado y en la página\"", buf);
                    sendCommand(TXbuffer);
//                    SendLHlist();
//                } else
//                if (RXbuffer[1]==0xF2){
...
static void terminate(int sig)
{
    ...
    sendCommand("page 0");
    sendCommand("dim=50");
    sprintf(TXbuffer,"t0.txt=\"NextionDriver v%s\"",NextionDriver_VERSION);
    sendCommand(TXbuffer);
